$(function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if(response.code === 200) {
                    tbUsers.rows[currentRow].cells[8].children[0].innerHTML = status;
                    if('activate' === status){
                        tbUsers.rows[currentRow].cells[8].children[0].style.color = '#00b300';
                    }else{
                        tbUsers.rows[currentRow].cells[8].children[0].style.color = '#f00';
                    }
    ...
    $('#seCategory').change(function () {
        ...
        if(key.length >0){
            for(var i =0 ; i < rowsLength; i ++){
                var content = tbUsers.rows[i].cells[7].innerHTML;
                if(currentSeStatus.length > 0){
                    var statusContent = tbUsers.rows[i].cells[8].children[0].innerHTML;
    ...
    $('#seStatus').change(function () {
        ...
        if(key.length >0){
            for(var i =0 ; i < rowsLength; i ++){
                var content = tbUsers.rows[i].cells[8].children[0].innerHTML;
                if(currentSePlan.length > 0){
                    var planContent = tbUsers.rows[i].cells[7].innerHTML;
    ...
    function showDisplayRows() {
        for(var i =0 ; i < rowsLength; i ++){
            tbUsers.rows[i].style.display = "";
            if(currentSePlan.length > 0){
                var content1 = tbUsers.rows[i].cells[7].innerHTML;
                if (content1 !== currentSePlan) {
                    tbUsers.rows[i].style.display = "none";
                }
            }
            if(currentSeStatus.length > 0) {
                var content2 = tbUsers.rows[i].cells[8].children[0].innerHTML;
