$(function () {
    ...
    function showCurrentTotalInfoOfTableOrders() {
        ...
        for(var i =0 ; i < rowsLength; i ++){
            ...
            if(status !== 'none'){
                count ++;
                totalAmount += parseFloat(tbOrders.rows[i].cells[7].innerHTML);
                totalTxFee += parseFloat(tbOrders.rows[i].cells[8].innerHTML);
                totalDeposit += parseFloat(tbOrders.rows[i].cells[9].innerHTML);
                totalLdCommission += parseFloat(tbOrders.rows[i].cells[10].innerHTML);
                totalLdeCommission += parseFloat(tbOrders.rows[i].cells[11].innerHTML);
                totalDealerCommission += parseFloat(tbOrders.rows[i].cells[12].innerHTML);
            }
        }
